# 📊 Hệ Thống Hỗ Trợ Ra Quyết Định Đa Tiêu Chí AHP
...
*   **Quản lý Tiêu chí & Phương án Linh hoạt**:
...
*   **Ma trận So sánh Cặp Trực quan**:
    *   Giao diện thân thiện giúp nhập giá trị so sánh cặp cho các tiêu chí và giữa các phương án dựa trên từng tiêu chí cụ thể.
...
*   **Trực quan hóa Dữ liệu Mạnh mẽ**:
    *   Trình bày trọng số tiêu chí và điểm số phương án bằng biểu đồ Pie Chart và Bar Chart sinh động, giúp dễ dàng phân tích và đưa ra quyết định.
*   **Lịch sử Tính toán Toàn diện**:
    *   Lưu trữ toàn bộ lịch sử các phiên tính toán vào cơ sở dữ liệu MongoDB, cho phép xem lại chi tiết bất kỳ kết quả đã lưu trữ nào.
...
Để chạy ứng dụng này, bạn cần cài đặt các thành phần sau:
...
1.  **Tải xuống và cài đặt MongoDB Community Server** từ trang web chính thức.

2.  **Tạo thư mục dữ liệu cho MongoDB**: MongoDB cần một thư mục để lưu trữ dữ liệu. Theo mặc định, nó sử dụng `C:\data\db` trên Windows hoặc `/data/db` trên Linux/macOS. Bạn cần tạo thư mục này nếu nó chưa tồn tại:
...
3.  **Khởi động MongoDB Server (`mongod`)**: Mở một cửa sổ Terminal hoặc Command Prompt/PowerShell mới (không phải cửa sổ đang chạy môi trường ảo của bạn) và chạy lệnh `mongod`.
...
4.  **Kiểm tra cấu hình MongoDB trong `app.py`**: Ứng dụng sẽ cố gắng kết nối đến MongoDB trên `localhost` tại cổng `27017` (cổng mặc định). Nếu MongoDB của bạn chạy trên một địa chỉ hoặc cổng khác, bạn cần chỉnh sửa biến `MONGO_URI` trong file `app.py` của bạn.

## ▶️ Chạy Ứng Dụng

Sau khi hoàn tất các bước cài đặt và đảm bảo MongoDB đang chạy:

1.  Đảm bảo môi trường ảo đã được kích hoạt (kiểm tra `(venv)` ở đầu dòng lệnh).
2.  Đảm bảo MongoDB Server đang chạy (mở cửa sổ `mongod` riêng).
